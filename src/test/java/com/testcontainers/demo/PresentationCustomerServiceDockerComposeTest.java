...
@Disabled
@Testcontainers
public class PresentationCustomerServiceDockerComposeTest {

  @Container
  public static ComposeContainer  environment = new ComposeContainer(new File("src/test/resources/docker-compose.yml"))
      .withExposedService("db", 5432, Wait.forLogMessage("database system is ready to accept connections", 0))
  ...
  void setUp() {
    DBConnectionProvider connectionProvider = new DBConnectionProvider(
        "jdbc:postgresql://" + environment.getServiceHost("db", 5432) + ":"
            + environment.getServicePort("db", 5432) + "/test",
        "postgres",
        "postgres");
